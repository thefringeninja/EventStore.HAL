...
namespace EventStore.HAL {
	internal class LinksConverter : JsonConverter<IEnumerable<Link>> {
		public override IEnumerable<Link> Read(ref Utf8JsonReader reader, Type typeToConvert,
		...
		private void WriteLink(Utf8JsonWriter writer, in Link link) {
			...
			writer.WriteString("type", Constants.MediaTypes.HalJson);
			writer.WriteString("title", link.Rel);
